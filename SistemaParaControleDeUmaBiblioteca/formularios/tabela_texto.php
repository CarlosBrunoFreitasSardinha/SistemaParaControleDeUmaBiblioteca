<?php 
	...
	$idiom = $_POST['idioma'];//tabela-texto
	$edica = $_POST['edicao'];//tabela-texto
	$imprent = $_POST['imprenta'];//tabela-texto
	$coleca = $_POST['colecao'];//tabela-texto
...
$sql = mysql_query("UPDATE texto
					SET  edicao_texto='$edica', idioma_texto='$idiom', imprenta_texto='$imprent', tipo_colecao_texto='$coleca', tombo='$tomb', ano_texto= '$an'
					WHERE id_acervo=$acerv ") or die("Erro no comando SQL:".mysql_error());
	...
	include ("include/conect_banco.php");
	   $sql= mysql_query("SELECT * FROM texto") or die ("Erro no Comando SQL".mysql_error());
	   $linhas=mysql_num_rows($sql);
	   for ($i=0;$i<$linhas;$i++){
	   		$SQL=mysql_fetch_array($sql);
			$id_acervo[$i] = $SQL['id_acervo'];//tabela livro
			$isbn[$i] = $SQL['imprenta_texto'];//tabela 
			$editora[$i] = $SQL['idioma_texto'];//tabela livro
	...
	for ($i=0;$i<$linhas;$i++){		
	   ...
	   $sql= mysql_query("SELECT * FROM acervo WHERE id_acervo='$id_acervo[$i]' ") or die ("Erro no Comando SQL".mysql_error());
		...
		if($i==0){echo "<table border=\"0\" cellspacing=\"0\" cellpading=\"0\">
							<tr>
								<td colspan=\"9\" width=\"560\" bgcolor=\"#000000\"><font color=\"#ffffff\"><b><center>Textos</td></tr>";
							echo "<tr>
						...
						if (($i%2)=='1'){
										echo "<tr onMouseOut=\"this.style.background=''\"  onmouseover=\"this.style.background='#9EDAB0';\" bgcolor=\"#EBEBEB\">
												<td><center><a href=\"index.php?acao=$ADT&&id_l=".$id_acervo[$i]."\"><img src=\"images/b_edit.png\" alt=\"Alterar\"></a></td>
