...
<table>
	<?php
		...
		$pesquisa=mysql_query("SELECT * FROM curso WHERE id_curso='$id_qualquer'")or die("Erro no Comando SQL: ".mysql_error());
		$s=mysql_fetch_array($pesquisa);
		$nome = $s['descricao_curso'];
		 echo "<form action=\"index.php?acao=$TCC\" method=\"post\">";?></table>
<table>
   <tr>
   		<td width="180"> Informe Titulo da Curso</td>
		<td><?php
