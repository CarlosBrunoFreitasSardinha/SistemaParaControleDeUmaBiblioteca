<?php 
	...
	include ("include/conect_banco.php");
	   $sql= mysql_query("SELECT * FROM video") or die ("Erro no Comando SQL".mysql_error());
	   $linhas=mysql_num_rows($sql);
	   for ($i=0;$i<$linhas;$i++){
	   		$SQL=mysql_fetch_array($sql);
			$id_acervo[$i] = $SQL['id_acervo'];//tabela livro
			$isbn[$i] = $SQL['id_tipo_midia'];//tabela 
			$editora[$i] = $SQL['cutter_video'];//tabela livro
	...
	for ($i=0;$i<$linhas;$i++){		
	   ...
	   $sql= mysql_query("SELECT * FROM acervo WHERE id_acervo='$id_acervo[$i]' ") or die ("Erro no Comando SQL".mysql_error());
		...
		if($i==0){echo "<table width=\"560\" border=\"0\" cellspacing=\"0\" cellpading=\"0\">
							<tr>
								<td colspan=\"8\" bgcolor=\"#000000\"><font color=\"#ffffff\"><b><center>Mídias</td></tr>";
							echo "<tr>
						...
						if (($i%2)=='1'){
										echo "<tr onMouseOut=\"this.style.background=''\"  onmouseover=\"this.style.background='#9EDAB0';\" bgcolor=\"#EBEBEB\">
												<td><center><a href=\"index.php?acao=$ADM&&id_l=".$id_acervo[$i]."\"><img src=\"images/b_edit.png\" alt=\"Alterar\"></a></td>
